...
public class ConnectFourGame implements ConnectFourGameInterface {
    ...
    connecto.ConnectFourPlayerInterface player1;
    connecto.ConnectFourPlayerInterface player2;

    ConnectFourGame(connecto.ConnectFourPlayerInterface player1, connecto.ConnectFourPlayerInterface player2){
    ...
    /**
     * This method takes the input from user for the column
     * player wants to take turn.
     *
     * @param player player1 or player2
     */
    private void takeTurn(connecto.ConnectFourPlayerInterface player) {
        System.out.println("Player " + player.getPlayerNumber() + ": " +
                "" + player.getName() + " " + "select a column");

        columnSelectedByPlayer = scanner.nextInt();

        //check if column selected by player is greater than the game board
        while (columnSelectedByPlayer > gameBoard[0].length-1) {
            System.out.println("Column number can't be greater than the game " +
                    "board. Choose column within game board");

            columnSelectedByPlayer = scanner.nextInt();

        }
        // check if selected column is already full
        while (gameBoard[0][columnSelectedByPlayer] != '.') {
            System.out.println("Column is already full on game " +
                    "board. Choose different column on game board");
